...
public class GovernmentIntelligenceServiceImpl implements GovernmentIntelligenceService {
    ...
    public void calculateSkillGaps(Long districtId, Integer year) {
        ...
        for (MarketDemand demand : marketDemandRepository.findByDistrict_IdAndYear(districtId, year)) {
            int capacity = trainingSupplyRepository
                    .findByJobRole_IdAndSkill_IdAndYear(
                            demand.getJobRole().getId(), demand.getSkill().getId(), year)
                    .stream().mapToInt(TrainingSupply::getCapacity).sum();

            int demandValue = demand.getDemand();
            int gap = Math.max(demandValue - capacity, 0);
            double priority = calculatePriorityScore(demandValue, gap);

            SkillGapAnalysis entity = skillGapAnalysisRepository
                    .findByDistrict_IdAndJobRole_IdAndSkill_IdAndYear(
                            districtId, demand.getJobRole().getId(), demand.getSkill().getId(), year)
    ...
    private double calculatePriorityScore(int demand, int gap) {
        if (demand <= 0) return 0.0;
        double demandScore = Math.min((demand / 1000.0) * 100.0, 100.0);
        double gapScore = Math.min(((double) gap / demand) * 100.0, 100.0);
        return Math.round((0.5 * demandScore + 0.5 * gapScore) * 100.0) / 100.0;
    }

    private Integer latestYear() {
        MarketDemand latest = marketDemandRepository.findTopByOrderByYearDesc();
        return latest == null ? null : latest.getYear();
    }

    private void refreshDistrictIfPossible(Long districtId) {
        Integer year = latestYear();
        if (year != null) calculateSkillGaps(districtId, year);
    }

    @Override
    @Transactional(readOnly = true)
    public PageSkillGapResponse getDistrictSkillGaps(Long districtId, int page, int limit) {
        if (!districtRepository.existsById(districtId))
            throw new ResourceNotFoundException("District not found with id: " + districtId);

        // Existing persisted analysis is the source for this read API.
        Pageable pageable = PageRequest.of(Math.max(page - 1, 0), Math.min(limit, 100));
        Page<SkillGapAnalysis> result = skillGapAnalysisRepository.findByDistrict_Id(districtId, pageable);

        return new PageSkillGapResponse(
                result.getContent().stream().map(mapper::toSkillGapResponse).toList(),
                page, limit, result.getTotalElements(), result.getTotalPages());
    }

    @Override
    @Transactional(readOnly = true)
    public DistrictDashboardResponse getDistrictDashboard(Long districtId) {
        District district = districtRepository.findById(districtId)
                .orElseThrow(() -> new ResourceNotFoundException("District not found with id: " + districtId));

        List<SkillGapAnalysis> gaps = skillGapAnalysisRepository.findByDistrict_Id(districtId);
        Integer latest = latestYear();
        if (gaps.isEmpty() && latest != null) {
            // read-only dashboard does not mutate the database; calculate values in memory
            gaps = calculateInMemory(districtId, latest);
        }

        int critical = (int) gaps.stream().filter(g -> g.getPriorityScore() >= 80).count();
        int priority = (int) gaps.stream().filter(g -> g.getPriorityScore() >= 60).count();
        int capacity = gaps.stream().mapToInt(SkillGapAnalysis::getTrainingCapacity).sum();

        return new DistrictDashboardResponse(
                district.getId(), district.getName(), critical, priority, capacity,
                0, 0, 0, 0, 0.0);
    }

    private List<SkillGapAnalysis> calculateInMemory(Long districtId, Integer year) {
        return marketDemandRepository.findByDistrict_IdAndYear(districtId, year).stream().map(d -> {
            int capacity = trainingSupplyRepository
                    .findByJobRole_IdAndSkill_IdAndYear(d.getJobRole().getId(), d.getSkill().getId(), year)
                    .stream().mapToInt(TrainingSupply::getCapacity).sum();
            int gap = Math.max(d.getDemand() - capacity, 0);
            return SkillGapAnalysis.builder()
                    .district(d.getDistrict()).jobRole(d.getJobRole()).skill(d.getSkill())
                    .year(year).demand(d.getDemand()).trainingCapacity(capacity)
                    .gap(gap).priorityScore(calculatePriorityScore(d.getDemand(), gap)).build();
        }).toList();
    }
    ...
    public StateDashboardResponse getStateDashboard() {
        List<SkillGapAnalysis> allGaps = skillGapAnalysisRepository.findAll();
        Integer latest = latestYear();
        if (allGaps.isEmpty() && latest != null) {
            allGaps = marketDemandRepository.findByYear(latest).stream().map(d -> {
                int capacity = trainingSupplyRepository
                        .findByJobRole_IdAndSkill_IdAndYear(d.getJobRole().getId(), d.getSkill().getId(), latest)
                        .stream().mapToInt(TrainingSupply::getCapacity).sum();
                int gap = Math.max(d.getDemand() - capacity, 0);
                return SkillGapAnalysis.builder()
                        .district(d.getDistrict()).jobRole(d.getJobRole()).skill(d.getSkill())
                        .year(latest).demand(d.getDemand()).trainingCapacity(capacity)
                        .gap(gap).priorityScore(calculatePriorityScore(d.getDemand(), gap)).build();
            }).toList();
        }

        int critical = (int) allGaps.stream().filter(g -> g.getPriorityScore() >= 80).count();
        int emerging = (int) allGaps.stream().map(SkillGapAnalysis::getSkill).distinct().count();
        int atRisk = (int) allGaps.stream().filter(g -> g.getPriorityScore() >= 80)
                .map(g -> g.getDistrict().getId()).distinct().count();
        List<SkillGapResponse> top = allGaps.stream()
                .sorted(Comparator.comparing(SkillGapAnalysis::getPriorityScore).reversed())
                .limit(10).map(mapper::toSkillGapResponse).toList();

        // These two metrics require curriculum/market-history semantics not stored in current tables.
        return new StateDashboardResponse(critical, emerging, atRisk, 0, 0, top);
    }
}
